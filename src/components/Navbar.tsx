...
import { Calendar, Menu, X, ChevronRight, Rocket, Phone, Mail, Linkedin } from 'lucide-react';
...
export default function Navbar({ onOpenBooking, onScrollTo }: NavbarProps) {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 transition-all">
      {/* Top micro-announcement / positioning bar (Optimized for Mobile) */}
      <div className="bg-slate-950 text-slate-200 text-xs py-1.5 sm:py-2 px-3 sm:px-4 border-b border-slate-800">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 truncate">
            ...
            <span className="font-semibold text-emerald-400 shrink-0">B2B Strateji:</span>
            <span className="text-slate-300 truncate text-[11px] sm:text-xs">
              Dijital Dönüşüm, Yapay Zekâ & InsurTech Mimarisi
            </span>
          </div>
          <div className="hidden md:flex items-center gap-4 text-slate-400 shrink-0 text-xs">
            <span>Marmara Sigortacılık '18 • Bağımsız Danışman</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          <div className="hidden sm:flex items-center gap-2.5">
            <button
              onClick={() => handleNavClick('acente-paketi')}
          ...
          <div className="bg-white border-b border-slate-200 px-4 pt-3 pb-6 space-y-4 shadow-2xl max-h-[calc(100vh-100px)] overflow-y-auto">
            ...
            <div className="pt-3 border-t border-slate-100 flex flex-col gap-2.5">
              ...
              >
                ...
                <span>Ücretsiz 30 Dk Keşif Randevusu Al</span>
              </button>

              <button
                onClick={() => handleNavClick('acente-paketi')}
                className="w-full py-3 text-center text-xs font-bold text-emerald-900 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-xl cursor-pointer flex items-center justify-center gap-2"
              >
                <Rocket className="w-4 h-4 text-emerald-600" />
                <span>Acentenizi Dijitalleştirin Paketini İncele</span>
              </button>
            </div>
